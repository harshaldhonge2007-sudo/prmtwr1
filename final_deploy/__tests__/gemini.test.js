...
describe('GeminiService', () => {
    it('should maintain session history and return a response', async () => {
        ...
        const response = await geminiService_1.default.getChatResponse(sessionId, message);
    ...
    it('should handle API errors gracefully', async () => {
        ...
        jest.spyOn(geminiService_1.default, 'getChatResponse').mockRejectedValueOnce(new Error('API Failure'));
        await expect(geminiService_1.default.getChatResponse(sessionId, message)).rejects.toThrow('API Failure');
    });
});
